...
get data from D16, matched to rotation sample of McQuillan. since we care about extending flare rates to entire populations of stars, we have to consider null detections. all Kepler LC's were searched by D16, incl. flare injection tests. so even for stars w/o flares, we have upper limits on flare rates we could have measured.
...
three examples of flare stars used in this analysis are shown in Figure \ref{fig:ffd1}. these example stars will be followed through the analysis of the paper, but have not been cherry-picked, just selected to demonstrate a range of flare rates
...
\caption{
Three examples of flare frequency distributions from paper 1. All three examples demonstrate breaks from a single power law frequency distribution at high flare energies.
}
\label{fig:ffd1}
\end{figure*}
...
we note some contamination exists in the original 4k stars from D16, some binaries and RR Lyr which {\tt appaloosa} incorrectly flagged as flare candidates. While these edge cases do not dominate the sample, we urge caution in not blindly assuming every candidate object from D16 is a bona fide flare star. 
...
Our discussion in many ways mirrors the review on flare activity by \citet{kunkel1975}, which explored different methods for quantifying and comparing flare activity from heterogeneous photometric studies. 
