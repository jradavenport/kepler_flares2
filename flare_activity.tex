...
Each line shown is actually the mean FFD for a single star using all available quarters of \Kepler data, the same as the average shown in Figure \ref{fig:ffd1}.
The FFD lines are colored by the rotation period reported by \citet{mcquillan2014}, with increasing rotation periods from red to blue. The panels of Figure \ref{fig:meanffd} represent slices in mass space of the four dimensional data space.
