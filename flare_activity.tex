...
In Figure \ref{fig:rossby2} we demonstrate the specific flare rate $R_{35}$ for the same sample of stars shown in Figure \ref{fig:rossby1}. Here we explore both the dependence on mass and Rossby number (top) and the observed color and rotation period (bottom). The energy of $10^{35}$ erg was selected as the average event energy observed in the \citet{davenport2016} flare census. For stars of a given mass or color, the specific flare rate declines as expected, consistent with the results from the previous section. 
However, a gradient in $R_{35}$ is seen as a function of both mass and color. This is due to the specific flare rates being lower for low-mass stars, reproducing the previously known result that while low-mass stars produce a high rate of observable flares and can produce a large fraction of their luminosity in flares, the actual rate of events with solar-type flare energies is low. 
